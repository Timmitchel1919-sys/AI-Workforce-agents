 ...
 */
import type {
  EnvironmentCodeRoute,
  EnvironmentRequirement,
  SoftwareFactoryEnvironmentProvider,
...
const CODE_REQUIREMENTS: Readonly<Record<string, EnvironmentRequirement>> = {
  docker: {
...
export function createSoftwareFactoryEnvironmentProvider(
  ...
  const requirementFor = (code: string): EnvironmentRequirement | null => {
    if (code === "none") return null;
    return CODE_REQUIREMENTS[code] ?? null;
  };
  return {
    requirementFor,
    route(codes): readonly EnvironmentCodeRoute[] {
      return codes.map((code): EnvironmentCodeRoute => {
        if (code === "none") {
        ...
        try {
          ...
          return {
            ...
            outcome: {
              outcome: "UNSUPPORTED",
              reason:
                `environment code "${code}" could not be routed: ` +

...
export class EnvironmentRouter {
  ...
  route(requirement: EnvironmentRequirement): EnvironmentRoutingOutcome {
    ...
    if (evidence.selectedInstanceId !== undefined) {
      ...
      if (instance && host) return { outcome: "ROUTED", instance, host };
    }
    if (
      evidence.outcome === "REQUIRES_PROVISIONING" &&
      evidence.supportingDescriptorId !== undefined
    ) {
      return {
        outcome: "REQUIRES_PROVISIONING",
        reason:
          `descriptor "${evidence.supportingDescriptorId}" is supported but no ` +
  ...
  evaluate(requirement: EnvironmentRequirement): EnvironmentMatchEvidence {
    ...
    const hosts = new Map(
      this.registry.listHosts().map((h) => [h.hostId, h] as const),
    );
    const evaluated = this.registry
      .listInstances()
      .map((instance) => ({
        instance,
        evidence: evaluateInstance(
          requirement,
          instance,
          hosts.get(instance.hostId),
        ),
      }))
    ...
    const supportingDescriptorId = provisionableDescriptorId(
      this.registry,
      requirement,
    );
...
function evaluateInstance(
...
): EnvironmentCandidateEvidence {
  const reasons: EnvironmentRejectionReason[] = [];
  if (instance.availability !== "available") {
    reasons.push("instance_unavailable");
  }
  if (!host) reasons.push("host_unknown");
  else if (host.availability !== "available") reasons.push("host_unavailable");
  if (
    requirement.descriptorId !== undefined &&
  ...
  if (host && requirement.os?.os !== undefined) {
    if (host.os.os !== requirement.os.os) reasons.push("os_mismatch");
  }
  ...
  const available = new Set(
    instance.capabilities.filter((c) => c.available).map((c) => c.capability),
  );
  const required = requirement.requiredCapabilities ?? [];
  const matchedCapabilities: CapabilityId[] = required.filter((c) =>
    available.has(c),
  );
  const missingCapabilities: CapabilityId[] = required.filter(
    (c) => !available.has(c),
  );
  if (missingCapabilities.length > 0) reasons.push("missing_capability");

  const shortfalls = toolchainShortfalls(
    requirement.toolchains ?? [],
    instance.toolchains,
  );
  for (const shortfall of shortfalls) {
    if (!reasons.includes(shortfall.reason)) reasons.push(shortfall.reason);
  }
...
function compareCandidates(
  a: EnvironmentInstance,
  b: EnvironmentInstance,
): number {
...
function provisionableDescriptorId(
  registry: EnvironmentRegistry,
  requirement: EnvironmentRequirement,
): string | undefined {
  if (requirement.descriptorId !== undefined) return requirement.descriptorId;

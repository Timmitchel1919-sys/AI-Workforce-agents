 ...
 */
export function defineBuildTool(spec: BuildToolSpec): {
  ...
  if (FORBIDDEN_EXECUTABLES.has(spec.executableId.toLowerCase())) {
    throw new ValidationError(
      `${spec.executableId} is a command interpreter and is never a build tool`,
    );
  }
  ...
  const operations = spec.commands.map((command) => {
    ...
    for (const arg of command.argv) {
      if (
        ...
        throw new ValidationError(
          `${command.operationId}: argument "${String(arg).slice(0, 40)}" is not allowed`,
        );
      }
    }
    command.requiredPaths?.forEach((p) => resolveWorkspacePath(p));
    const access = command.workspaceAccess ?? "read";
    const op: ExecutionOperationDefinition = {
  ...
  const tool: ExecutionToolDefinition = {
    toolId: spec.toolId,
    version: "1.0.0",
    ...
    executable: {
      ...
      operations: Object.fromEntries(
        spec.commands.map((c) => [
          c.operationId,
          c.argv.map((value) => ({ kind: "literal" as const, value })),
        ]),
      ),
 ...
 */
export const BUILD_PROFILE_PRESETS: Readonly<Record<string, BuildToolSpec>> = {
  dotnet: {
    toolId: "dotnet-build",
    executableId: "dotnet",

...
function violation(detail: string): ExecutionDeniedError {
 ...
 */
export function resolveWorkspacePath(requested: unknown): string {
  if (typeof requested !== "string") throw violation("path must be a string");
  if (requested.length === 0) throw violation("path must not be empty");
  if (requested.length > MAX_PATH_LENGTH) throw violation("path is too long");

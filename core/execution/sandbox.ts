...
export class SandboxRegistry {
  ...
  select(
    ...
    needs: SandboxNeeds = DEFAULT_NEEDS,
  ): SandboxProvider | undefined {
    const isolation = (p: SandboxProvider) =>
      Number(p.capabilities.filesystemIsolation) +
      Number(p.capabilities.networkIsolation);
    return [...this.providers.values()]
      .sort(
        (a, b) =>
          isolation(b) - isolation(a) ||
      ...
      .find((p) => {
        ...
        if (needs.requiredProviderId !== undefined) {
          if (p.providerId !== needs.requiredProviderId) return false;
        } else if (caps.requiresRouting) {
          return false;
        }
        if (!p.isAvailableFor(environmentInstanceId)) return false;
        if (
          needs.executableId !== undefined &&
          caps.executables !== undefined &&
          !caps.executables.includes(needs.executableId)
        ) {
          return false;
        }
        if (!caps.networkModes.includes(network.mode)) return false;
        if (
          !REQUIRED_LIMIT_KEYS.every((k) => caps.enforcedLimits.includes(k))
        ) {
          return false;
        }
        const needsNetwork =
          network.mode !== "deny_all" || needs.networkAccess !== "none";
        if (needsNetwork && !caps.networkIsolation) return false;
        if (caps.filesystemIsolation && caps.networkIsolation) return true;
        // Non-isolating providers — only with an explicit policy opt-in:
        // (a) host diagnostics: no workspace, no network (EO-4.2);
        if (
          needs.hostProcessAllowed &&
        ...
        return (
          needs.trustedHostBuildAllowed === true &&
...
const DEFAULT_NEEDS: SandboxNeeds = {

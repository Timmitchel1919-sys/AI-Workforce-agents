...
export class InMemoryOperatorAccountStore implements OperatorAccountStore {
  private readonly accounts = new Map<string, OperatorAccount>();
  private activeAdmins = 0;
  private bootstrapOperatorId: string | undefined;
  ...
  private apply(change: OperatorAccountChange): OperatorAccountCommitResult {
    ...
    if (change.kind === "bootstrap") {
      ...
      if (this.activeAdmins > 0) throw new BootstrapLockedError();
      this.accounts.set(id, structuredClone(change.account));

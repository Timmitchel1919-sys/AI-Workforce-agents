...
export function planStages(
  ...
  environments: readonly DraftEnvironmentRequirement[],
  dependencies: readonly DependencyRequirement[],
  catalog: TechnologyCatalog,
): StagePlan {
  const agentRequirements: AgentRequirement[] = [];
  const build: BuildStage[] = [];
  const tests: TestStage[] = [];
  const sastComponents: string[] = [];
  ...
  for (const component of analysis.components) {
    ...
    const environment = environments.find((e) =>
      e.componentIds.includes(component.componentId),
    );
    ...
    agentRequirements.push({
      ...
      requiredCapabilities: unique(
        profiles.flatMap((p) => p.agentCapabilities),
      ),
    ...
    const types = new Set<TestType>(profiles.flatMap((p) => p.testTypes));
...
function planDeployments(
  ...
  const out: DeploymentRequirement[] = [];
  for (const intent of request.deployments ?? []) {
    ...
    out.push({
      ...
      preDeploymentGates: [],
      rollbackRequired:
        intent.stage === "production" || intent.stage === "staging",
    });
  }
  return out;
}
...
function planApprovals(
  ...
  const out: PlanApprovalRequirement[] = [];

...
export interface TechnologyProfile {
  ...
  toolchainDependencies?: readonly { from: ToolchainKind; to: ToolchainKind }[];
  os?: { os?: OsName };
...
const v = (major: number, minor = 0, patch = 0) => ({ major, minor, patch });

/** Bump when a profile changes so replans detect catalog changes. */
export const TECHNOLOGY_CATALOG_VERSION = "2026-09-23.1";

export const TECHNOLOGY_PROFILES: readonly TechnologyProfile[] = [
  {
    id: "react_typescript",
...
export class TechnologyCatalog {
  private readonly byId = new Map<string, TechnologyProfile>();

  constructor(
    profiles: readonly TechnologyProfile[] = TECHNOLOGY_PROFILES,
    readonly version: string = TECHNOLOGY_CATALOG_VERSION,
  ) {
    for (const profile of profiles) {
      if (this.byId.has(profile.id)) {
        throw new ValidationError(
          `duplicate technology profile: ${profile.id}`,
        );
      }
      ...
      for (const dep of profile.toolchainDependencies ?? []) {
        if (!kinds.has(dep.from) || !kinds.has(dep.to)) {
          throw new ValidationError(
            `technology ${profile.id}: toolchain dependency references an undeclared toolchain`,
          );
        }
      }

...
export interface GraphProjectionSources {
  ...
  agentOps?: { isEnabled(agentId: string): boolean };
...
export function resolveBounds(options: GraphQueryOptions): ResolvedGraphBounds {
  const clamp = (
  ...
  return {
    depth: clamp(
      options.depth,
      GRAPH_LIMITS.defaultDepth,
      GRAPH_LIMITS.maxDepth,
    ),
    maxNodes: clamp(
      options.maxNodes,
      GRAPH_LIMITS.defaultMaxNodes,
      GRAPH_LIMITS.maxNodes,
    ),
    maxEdges: clamp(
      options.maxEdges,
...
export class GraphBuilder {
  ...
  readonly edges = new Map<string, WorkforceGraphEdge>();
  ...
  merge(fragment: GraphFragment): void {
    for (const n of fragment.nodes) this.addNode(n);
    for (const e of fragment.edges) this.addEdge(e);
  }
}
...
export function boundedView(
  ...
  if (roots.length > 0) {
    ...
    for (
      let level = 0;
      level < bounds.depth && frontier.length > 0;
      level += 1
    ) {
      const next: string[] = [];
    ...
    if (!truncated) {
      for (const id of visited) {
        for (const neighbour of adjacency.get(id) ?? []) {
          if (!visited.has(neighbour)) truncated = true;
        }
      }
    }
  }
...
export class WorkforceGraphProjectionService {
  constructor(
    ...
    private readonly sfOrchestrator: SoftwareFactoryOrchestrator | undefined,
    private readonly sources: GraphProjectionSources = {},
  ...
  public getProjection(options: GraphQueryOptions): WorkforceGraphProjection {
    ...
    const requestedRoot =
      options.rootNodeId && builder.nodes.has(options.rootNodeId)
        ? options.rootNodeId
        : undefined;
    ...
    const effectiveRoot =
      requestedRoot && nodes.some((n) => n.id === requestedRoot)
        ? requestedRoot
        : undefined;
    ...
    const view = boundedView(
      ...
      selection.edges,
      selection.rootIds,
      scoped ? bounds : { ...bounds, depth: Number.POSITIVE_INFINITY },
    );
  ...
  private buildBaseGraph(
    ...
    for (const t of tasks) {
      if (t.assignedAgentId) {
        tasksPerAgent.set(
          t.assignedAgentId,
          (tasksPerAgent.get(t.assignedAgentId) ?? 0) + 1,
        );
      }
    }
    ...
    for (const program of this.sfOrchestrator?.overview().programs ?? []) {
      if (program.projectId !== projectId) continue;
    ...
    for (const task of tasks) {
      const upstreamIds = (task.dependencies ?? []).filter((d) =>
        statusById.has(d),
      );
      b.addNode({
        id: `task-${task.id}`,
      ...
      for (const depId of task.dependencies ?? []) {
        ...
        b.addEdge({
          ...
          status:
            statusById.get(depId) !== "completed" &&
  ...
  private routesForProject(projectId: string): TaskEnvironmentRoutingSummary[] {
  ...
  private addWorkflows(
    ...
    const workflows: Workflow[] = (this.sources.workflows?.list() ?? [])
      .filter((w) => w.projectId === projectId)
      .sort(byId);
    for (const wf of workflows) {
      const total = wf.taskRecords.length;
      const done = wf.taskRecords.filter(
      ...
      for (const record of wf.taskRecords) {
        if (!record.taskId || !taskIds.has(record.taskId)) continue;
        b.addEdge({

...
function ensureDAG(tasks: readonly Task[]): void {
  ...
  for (const task of tasks) {
    ...
    if (!adjacencyList.has(task.id)) adjacencyList.set(task.id, []);
  }
...
export class SoftwareFactoryOrchestrator {
  private readonly programs = new Map<string, SoftwareFactoryProgram>();
  ...
  private readonly executionAliases = new Map<string, string>();
  ...
  createWorkstream(
    ...
    if (!program) throw new NotFoundError(`unknown program: ${programId}`);
  ...
  async tick(): Promise<void> {
    for (const workstream of this.workstreams.values()) {
      ...
      for (const taskId of [...workstream.tasks]) {
        ...
        if (!this.environmentGateAllows(task)) continue;

        const submitted = await this.orchestrator.submit(
          this.toSubmitDraft(task),
        );
  ...
  overview(): SoftwareFactoryOverview {
    const programs: ProgramSummary[] = [...this.programs.values()].map(
      (program) => {
        let taskCount = 0;
  ...
  private programWorkstreams(programId: string): Workstream[] {
    const program = this.programs.get(programId);
    if (!program) return [];
   ...
   */
  private environmentGateAllows(task: Task): boolean {
    const codes = task.environmentRequirements ?? [];
    if (codes.length === 0) return true;
    return this.environments
      .route(codes)
      .every(
        (route) => route.code === "none" || route.outcome.outcome === "ROUTED",
      );
  }
   ...
   */
  private toSubmitDraft(task: Task): TaskDraft {
    return {
      type: task.type,

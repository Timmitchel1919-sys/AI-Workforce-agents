 ...
 */
import { BootstrapLockedError, ValidationError, } from "../contracts/index.js";
import { FirestoreOperatorAccountStore, firestoreRepository, isTransactionalFirestore, } from "../adapters/firebase/index.js";
...
export async function bootstrapInitialAdministrator(options) {
    ...
    if (!isTransactionalFirestore(firestore)) {
        throw new ValidationError("a Firestore client with transactions is required");
    }
    let user;
    try {
    ...
    try {
        ...
        if (!(error instanceof BootstrapLockedError))
            throw error;
        result = {
            outcome: "refused",
            message: "bootstrap is locked: an administrator already exists — grant access through Settings → Users & Access",
        };
    }
    const auditRepo = firestoreRepository(firestore, `${options.collectionPrefix ?? ""}audit_events`);
    for (const event of events)
        await auditRepo.upsert(event);
    return result;
}

...
import { DEFAULT_WORKSPACE_FILE_POLICY, ExecutionDeniedError, KNOWN_SECRET_VALUE_PATTERN, REQUIRED_LIMIT_KEYS, assertRealPathWithinRoot, canTransitionWorkspace, classifyWorkspacePath, resolveWorkspacePath, validateRepositoryReference, } from "../../contracts/index.js";
...
export class WorkspaceRepositorySandbox {
    options;
    providerId = "workspace-repository";
    ...
    constructor(options) {
        ...
        for (const repo of options.repositories) {
            ...
            if (repo.localPath && !path.isAbsolute(repo.localPath)) {
                throw new Error(`repository ${repo.repositoryId} localPath must be absolute`);
            }
    ...
    /* Lifecycle                                                      */
    /* -------------------------------------------------------------- */
    emit(projectId, action, data) {
        this.options.onEvent?.({ action, projectId, data });
    }
    setState(record, to) {
        if (record.view.state === to)
            return;
        if (!canTransitionWorkspace(record.view.state, to)) {
            deny("SANDBOX_FAILURE", `workspace cannot move from ${record.view.state} to ${to}`);
        }
    ...
    async start(spec) {
        ...
        if (record) {
            if (record.view.sessionId !== spec.sessionId ||
    ...
    assertLeaseFree(root, workspaceId) {
        const held = this.leases.get(root);
        if (held &&
            held.workspaceId !== workspaceId &&
            Date.parse(held.expiresAt) > Date.parse(this.clock())) {
            deny("WORKSPACE_CONFLICT", "the project workspace is leased by another write session");
        }
    }
    acquireLease(record, spec) {
        ...
        const expiresAt = new Date(Date.parse(now) + spec.limits.sessionTimeoutMs).toISOString();
    ...
    async create(spec) {
        ...
        if (!repo)
            deny("SANDBOX_UNAVAILABLE", "no repository is configured for this project");
        if (!repo.localPath) {
            deny("SANDBOX_UNAVAILABLE", "remote repository acquisition requires a secret-broker credential handle (not available in EO-4.3)");
        }
        const root = await realpath(repo.localPath).catch(() => deny("SANDBOX_UNAVAILABLE", "the repository path is unavailable"));
        if (!(await stat(root)).isDirectory())
            deny("SANDBOX_UNAVAILABLE", "the repository path is not a directory");
        const stateRoot = await realpath(this.options.stateRoot).catch(() => deny("SANDBOX_UNAVAILABLE", "the workspace state root is unavailable"));
        ...
        assertRealPathWithinRoot(stateRoot, statePath, process.platform === "win32" ? "win32" : "posix");
        ...
        for (const [a, b] of [
            ...
            try {
                assertRealPathWithinRoot(a, b, process.platform === "win32" ? "win32" : "posix");
        ...
        await writeFile(path.join(statePath, MARKER), JSON.stringify({
            workspaceId: spec.workspace.workspaceId,
            projectId: spec.projectId,
        }), { flag: "wx" }).catch(() => deny("SANDBOX_FAILURE", "the workspace state directory already exists"));
        const at = this.clock();
        const policy = this.options.projectPolicies?.[spec.projectId] ??
            this.options.policy ??
        ...
        try {
            ...
            if (hasGit && this.options.gitPath) {
                const status = await this.git(record, ["status", "--porcelain=v1", "-z", "--untracked-files=all"], 30_000, 4 * 1024 * 1024);
                for (const entry of parseStatus(status.text).entries) {
                    record.baseline.add(entry.path);
                    if (entry.fromPath)
                        record.baseline.add(entry.fromPath);
                }
                const head = await this.git(record, ["rev-parse", "HEAD"], 30_000, 1024).catch(() => undefined);
            ...
            if (this.leases.get(record.root)?.workspaceId === record.view.workspaceId) {
                this.leases.delete(record.root);
            }
            this.records.delete(record.view.workspaceId);
            await rm(statePath, { recursive: true, force: true }).catch(() => undefined);
            throw error;
        }
    ...
    async invoke(handle, invocation, options) {
        ...
        const outcome = (exitClass, text, extra = {}) => ({
            exitClass,
        ...
        try {
            const result = invocation.executableId === "workspace"
                ? await this.fileOperation(record, invocation.operationId, invocation.input ?? {})
                : invocation.executableId === "git"
                    ? await this.repositoryOperation(record, invocation, options)
                    : deny("TOOL_NOT_ALLOWED", "this sandbox runs only workspace and repository operations");
            ...
            if (record.view.state === "in_use") {
                this.setState(record, record.changeSet.entries.length > 0 ? "dirty" : "ready");
            }
        }
    }
    /* -------------------------------------------------------------- */
    ...
    resolve(record, requested) {
        const rel = resolveWorkspacePath(requested);
        const abs = rel === "." ? record.root : path.join(record.root, ...rel.split("/"));
        return { rel, abs };
    }
    /** realpath of `abs` (or of its nearest existing ancestor) stays inside the root. */
    async assertContained(record, abs) {
        let probe = abs;
        for (;;) {
            try {
                ...
                if (parent === probe)
                    deny("WORKSPACE_VIOLATION", "path cannot be resolved inside the workspace");
                probe = parent;
            }
        }
    }
    classify(record, rel, access) {
        const cls = classifyWorkspacePath(rel, record.policy);
        if (cls === "internal")
            deny("POLICY_DENIED", "repository internals (.git) are not accessible");
        if (cls === "secret")
            deny("POLICY_DENIED", "protected credential/secret file: content is never exposed");
        if (access === "write") {
            if (cls === "generated")
                deny("POLICY_DENIED", "generated/build output is not source; use artifact management");
            if (cls === "protected" &&
                !record.capabilities.has("filesystem.write.protected")) {
                deny("POLICY_DENIED", "protected path requires the filesystem.write.protected capability");
            }
        }
        return cls === "protected" ? "protected" : "normal";
    }
    requireWrite(record, capability) {
        if (record.view.mode !== "read_write")
    ...
    async currentHash(abs) {
        const info = await lstat(abs).catch(() => undefined);
        if (!info)
            return undefined;
        if (!info.isFile())
            deny("POLICY_DENIED", "only regular files can be modified (no symlinks or directories)");
        return sha256(await readFile(abs));
    }
    guardBaseline(record, rel) {
        if (record.baseline.has(rel) && !record.originals.has(rel)) {
            deny("WORKSPACE_CONFLICT", "the file has pre-existing uncommitted changes and is protected from overwrite");
        }
    }
    async remember(record, rel, abs) {
        if (record.originals.has(rel))
            return;
        if (record.originals.size >= record.policy.maxChangedFiles) {
            deny("RESOURCE_LIMIT", "the change set reached its maximum number of files");
        }
        const info = await lstat(abs).catch(() => undefined);
        if (!info) {
    ...
    async atomicWrite(record, abs, content) {
    ...
    recompute(record) {
        ...
        for (const [rel, original] of record.originals) {
            ...
            const risk = classifyWorkspacePath(rel, record.policy) === "protected"
                ? "protected"
                : "normal";
            if (original.hash === undefined && now !== undefined) {
                ...
                const renamedTo = [...record.movedFrom].find(([to, from]) => from === rel && record.current.get(to) === original.hash)?.[0];
                if (!renamedTo)
                    entries.push({
                        path: rel,
                        change: "deleted",
            ...
            else if (original.hash !== undefined &&
                now !== undefined &&
                original.hash !== now) {
                entries.push({
                    path: rel,
    ...
    /* File operations                                                */
    /* -------------------------------------------------------------- */
    async fileOperation(record, operationId, input) {
        ...
        switch (operationId) {
            case "workspace.file.read": {
                ...
                await this.assertContained(record, abs);
                const info = await stat(abs).catch(() => deny("INVALID_TOOL_INPUT", "file not found"));
                if (!info.isFile())
                    deny("INVALID_TOOL_INPUT", "not a regular file");
                if (info.size > policy.maxReadBytes)
                    deny("RESOURCE_LIMIT", `file exceeds the ${policy.maxReadBytes}-byte read limit`);
                const content = await readFile(abs);
            ...
            case "workspace.file.stat": {
                ...
                if (cls === "internal")
                    deny("POLICY_DENIED", "repository internals (.git) are not accessible");
                await this.assertContained(record, abs);
                const info = await lstat(abs).catch(() => deny("INVALID_TOOL_INPUT", "path not found"));
            ...
            case "workspace.file.list": {
                ...
                if (classifyWorkspacePath(rel, policy) === "internal")
                    deny("POLICY_DENIED", "repository internals (.git) are not accessible");
                await this.assertContained(record, abs);
                const entries = await readdir(abs, { withFileTypes: true }).catch(() => deny("INVALID_TOOL_INPUT", "directory not found"));
                const visible = entries
                    .filter((e) => !(rel === "." && e.name === ".git"))
                    .sort((a, b) => a.name.localeCompare(b.name));
                const offset = typeof input.offset === "number" ? input.offset : 0;
                const limit = Math.min(typeof input.limit === "number" ? input.limit : policy.maxListEntries, policy.maxListEntries);
            ...
            case "workspace.file.search":
                return {
                    data: await this.search(record, String(input.query ?? ""), input.path ?? "."),
                };
            case "workspace.file.create": {
                ...
                if (content.length > policy.maxWriteBytes)
                    deny("RESOURCE_LIMIT", `content exceeds the ${policy.maxWriteBytes}-byte write limit`);
                if (await lstat(abs).then(() => true, () => false))
                    deny("WORKSPACE_CONFLICT", "the file already exists (create never overwrites)");
            ...
            case "workspace.file.update": {
                ...
                if (before !== input.expectedHash)
                    deny("WORKSPACE_CONFLICT", "the file changed since it was read (expectedHash mismatch)");
                this.guardBaseline(record, rel);
            ...
            case "workspace.file.move": {
                ...
                if (before !== input.expectedHash)
                    deny("WORKSPACE_CONFLICT", "the file changed since it was read (expectedHash mismatch)");
                if (await lstat(to.abs).then(() => true, () => false))
                    deny("WORKSPACE_CONFLICT", "the target already exists (move never overwrites)");
    ...
    async secretScan(record) {
        ...
        while (queue.length > 0 && !truncated) {
            ...
            for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
                ...
                if (!info || info.size > policy.maxSearchFileBytes)
                    continue;
    ...
    async search(record, query, start) {
        const policy = record.policy;
        ...
        while (queue.length > 0 && !truncated) {
            ...
            const entries = await readdir(dir.abs, { withFileTypes: true }).catch(() => []);
            for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
                const childRel = dir.rel === "." ? entry.name : `${dir.rel}/${entry.name}`;
    ...
    async git(record, argv, timeoutMs, maxOutputBytes, signal = new AbortController().signal) {
    ...
    async repositoryOperation(record, invocation, options) {
        if (!record.gitDir)
            deny("ENVIRONMENT_UNAVAILABLE", "the workspace is not a git repository");
        const run = await this.git(record, invocation.argv, invocation.timeoutMs, Math.max(1024, options.maxOutputBytes), options.signal);
        if (run.cancelled)
            return { data: {}, exitClass: "cancelled" };
        if (run.timedOut)
            return { data: {}, exitClass: "timeout" };
        if (run.exitCode !== 0)
            return {
        ...
        switch (invocation.operationId) {
            case "repository.status": {
                ...
                return {
                    data: {
                        ...
                        truncated: run.truncated || parsed.entries.length > policy.maxListEntries,
                    },
                };
            }
            case "repository.changedFiles": {
                const parsed = parseStatus(run.text);
                const sessionPaths = new Set(record.changeSet.entries.flatMap((e) => [
                    e.path,
            ...
            case "repository.diff": {
                const { diff, omitted } = filterDiff(run.text, (p) => classifyWorkspacePath(p, policy) === "secret");
                const limit = Math.min(policy.maxReadBytes, Math.max(0, options.maxOutputBytes - 512));
     ...
     */
    async sourceFingerprint(projectId) {
        const root = await this.projectRoot(projectId);
        const home = await realpath(this.options.stateRoot);
        const policy = this.options.projectPolicies?.[projectId] ??
            this.options.policy ??
            DEFAULT_WORKSPACE_FILE_POLICY;
        const head = await this.git({ root, statePath: home }, ["rev-parse", "HEAD"], 30_000, 1024).catch(() => undefined);
        const baseRevision = head?.exitCode === 0 ? head.text.trim() : undefined;
        const status = await this.git({ root, statePath: home }, ["status", "--porcelain=v1", "-z", "--untracked-files=all"], 30_000, 8 * 1024 * 1024);
        ...
        for (const entry of parseStatus(status.text).entries) {
            ...
            const hash = await lstat(abs)
                .then(async (i) => i.isFile() ? sha256(await readFile(abs)) : "not-a-file")
                .catch(() => "deleted");
            files.push([entry.path, entry.code, hash]);
        }
        files.sort((a, b) => a[0].localeCompare(b[0]));
        return {
            fingerprint: sha256(JSON.stringify({ base: baseRevision ?? null, files })),
            ...(baseRevision ? { baseRevision } : {}),
            changedFiles: files.length,
        };
    }
    /** SHA-256 + size of a workspace-relative, non-secret file. */
    async digestFile(projectId, requested) {
        const root = await this.projectRoot(projectId);
        const rel = resolveWorkspacePath(requested);
        const policy = this.options.projectPolicies?.[projectId] ??
            this.options.policy ??
        ...
        const real = await realpath(abs).catch(() => deny("INVALID_TOOL_INPUT", "artifact not found"));
    ...
    setChangeSetStatus(workspaceId, status) {
    ...
    async rollback(workspaceId, sessionId) {
        const record = this.records.get(workspaceId);
        ...
        for (const [rel, original] of [...record.originals].reverse()) {
            ...
            const actual = await lstat(abs).then((i) => (i.isFile() ? readFile(abs).then(sha256) : "not-a-file"), () => undefined);
            if (actual !== expected) {
                skipped.push({
                    path: rel,
    ...
    async release(workspaceId, sessionId) {
        ...
        record.changeSet = {
            ...record.changeSet,
            status: record.changeSet.status === "open"
        ...
        try {
            ...
            const marker = JSON.parse(await readFile(path.join(real, MARKER), "utf8"));

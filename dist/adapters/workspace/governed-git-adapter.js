...
import { ExecutionDeniedError, resolveWorkspacePath, validateBranchName, } from "../../contracts/index.js";
import { runBoundedProcess } from "../execution/bounded-process-runner.js";
const NULL_DEVICE = "/dev/null";
const SHA = /^[0-9a-f]{40}([0-9a-f]{24})?$/;
export class GovernedGitAdapter {
    options;
    repos = new Map();
    constructor(options) {
        ...
        for (const repo of options.repositories) {
            ...
            if (!/^https:\/\/[^\s@]+$/.test(url) && !path.isAbsolute(url)) {
                throw new Error(`remote for ${repo.projectId} must be https or an absolute local path`);
            }
            if (/https:\/\/[^/]*@/.test(url)) {
    ...
    repo(projectId) {
        ...
        if (!repo)
            throw new ExecutionDeniedError("POLICY_DENIED", "no governed repository is configured for this project");
        return repo;
    }
    remoteId(projectId) {
        return this.repo(projectId).remote.remoteId;
    }
    async git(repo, args, credential) {
    ...
    async head(projectId) {
        ...
        const sha = (await this.git(repo, ["rev-parse", "--verify", "-q", "HEAD"])).text
            .trim()
            .split(/\s+/)[0];
        const branch = (await this.git(repo, ["symbolic-ref", "-q", "--short", "HEAD"])).text
            .trim()
            .split(/\s+/)[0];
        return {
    ...
    async commit(projectId, input) {
        const repo = this.repo(projectId);
        if (input.files.length === 0 || input.files.length > 500) {
            throw new ExecutionDeniedError("STAGING_CONFLICT", "a stage set needs 1-500 files");
        }
        const paths = [
            ...new Set(input.files
        ...
        if (paths.some((p) => p === "." || p.split("/")[0] === ".git")) {
            throw new ExecutionDeniedError("STAGING_CONFLICT", "repository internals are never staged");
        }
        const before = await this.head(projectId);
        if (input.expectedHead && before.sha !== input.expectedHead) {
            throw new ExecutionDeniedError("REVERIFICATION_REQUIRED", "the repository HEAD moved since the stage set was prepared");
        }
        // Bounded staging: ONLY these paths (additions, edits and deletions).
        const add = await this.git(repo, ["add", "--all", "--", ...paths]);
        if (add.exitCode !== 0)
            throw new ExecutionDeniedError("COMMIT_FAILED", "the stage set could not be staged");
    ...
    async readCommit(projectId, sha) {
        if (!SHA.test(sha))
            return undefined;
        const repo = this.repo(projectId);
    ...
    async remoteHead(projectId, branch, credential) {
        const repo = this.repo(projectId);
        const ref = `refs/heads/${validateBranchName(branch)}`;
        const out = await this.git(repo, ["ls-remote", "--heads", "--", repo.remote.url, ref], credential);
        if (out.exitCode !== 0)
            throw new ExecutionDeniedError("PUSH_FAILED", "the remote could not be read");
        const line = out.text.split(/\r?\n/).find((l) => l.trim().endsWith(ref));
        const sha = line?.split(/\s+/)[0];
        return sha && SHA.test(sha) ? sha : undefined;
    }
    async push(projectId, input) {
        const repo = this.repo(projectId);
        ...
        if (out.exitCode !== 0) {
            if (/rejected|non-fast-forward|fetch first/i.test(out.text)) {
                throw new ExecutionDeniedError("REMOTE_CHANGED", "the remote branch moved; nothing was overwritten");
            }
            throw new ExecutionDeniedError("PUSH_FAILED", "the push was refused by the remote");
        }

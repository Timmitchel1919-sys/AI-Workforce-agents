...
export class FirestoreExecutionPlanStore {
    firestore;
    plans;
    heads;
        ...
        try {
            await this.firestore.runTransaction(async (tx) => {
                ...
                if (change.kind === "new_revision") {
                    const previousRef = this.plans.doc(String(change.supersededRecord.id));
                    const previous = await tx.get(previousRef);
                    const data = previous.data();
                    if (headVersion !== change.expectedCurrentVersion ||

...
import { KNOWN_SECRET_VALUE_PATTERN, ValidationError, } from "../../contracts/index.js";
const REDACTED = "[redacted]";
const SECRET_SHAPES = new RegExp(KNOWN_SECRET_VALUE_PATTERN.source, "g");
function finalize(captured, totalBytes, maxBytes, knownSecrets) {
    let redactions = 0;
...
function killTree(child) {
    ...
    if (process.platform === "win32") {
        ...
        const taskkill = spawn(path.join(root, "System32", "taskkill.exe"), ["/pid", String(child.pid), "/T", "/F"], { shell: false, windowsHide: true, stdio: "ignore" });
...
export function validateBoundedProcessSpec(spec) {
    if (!path.isAbsolute(spec.executable.path)) {
        throw new ValidationError("executable path must be absolute (trusted composition)");
    }
    if (!path.isAbsolute(spec.cwd)) {
        throw new ValidationError("working directory must be provider-owned and absolute");
    }
...
export function runBoundedProcess(spec) {

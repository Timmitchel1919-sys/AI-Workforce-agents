 ...
 */
import { DEFAULT_TOOL_LIMITS, ENVIRONMENTS, MONEY_MIND_PROJECT_ID, validateMoneyMindInspectStructureInput, validateMoneyMindReadDocumentationInput, validateMoneyMindReadFileInput, validateMoneyMindRunTestsInput, } from "../../../contracts/index.js";
import { makeInMemoryTool, } from "../../tools/mock-tools.js";
import { MONEY_MIND_CAPABILITY_ACTIONS, agentsForMoneyMindCapability, } from "./money-mind-profile.js";
...
const ALL_ENVIRONMENTS = [...ENVIRONMENTS];
/** `research.fetch`'s convention is `{ reference }`; the adapter's is `{ path }`. */
function normalizeReadFileInput(input) {
    if (input &&
        typeof input === "object" &&
        !("path" in input) &&
        "reference" in input) {
        return { path: input.reference };
    }
    return input;
}
function baseDefinition(id, name, description, capability) {
    return {
        id,
        name,
        description,
...
export function makeMoneyMindTools(adapter) {
    const call = (op) => (input) => adapter.execute(op, input);
    const readProject = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.readProject, "Money Mind: Read Project", "Read the Money Mind project identity and profile.", "read"),
        requiredPermission: {
    ...
    const status = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.status, "Money Mind: Status", "Read structured V2 chapter status and feature-flag names.", "read"),
    ...
    const readTestResults = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.readTestResults, "Money Mind: Read Test Results", "Report whether an automated test suite is configured.", "read"),
        requiredPermission: {
    ...
    const readConfig = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.readConfig, "Money Mind: Read Configuration", "Read package name/version, npm scripts, and feature-flag names.", "read"),
        requiredPermission: {
    ...
    const readFile = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.readFile, "Money Mind: Read File", "Read one text file by repository-relative path (sandboxed). Accepts " +
            "`reference` as an alias for `path` so it doubles as the Research " +
            "Agent's fetch tool.", "read"),
    ...
    const test = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.test, "Money Mind: Run Tests", "Run one allowlisted, existing npm script (build/lint/test/typecheck).", "execute"),
    ...
    const inspect = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.inspect, "Money Mind: Inspect Structure", "List the repository's directory structure (paths + type only).", "read"),
        requiredPermission: {
    ...
    const readDocs = makeInMemoryTool({
        ...baseDefinition(MONEY_MIND_TOOL_IDS.readDocs, "Money Mind: Read Documentation", "Read, and optionally keyword-search, the project's documentation set.", "execute"),
        requiredPermission: {

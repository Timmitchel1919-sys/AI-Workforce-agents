 ...
 */
import { MONEY_MIND_PROJECT_ID, NotFoundError, ValidationError, validateMoneyMindInspectStructureInput, validateMoneyMindReadDocumentationInput, validateMoneyMindReadFileInput, validateMoneyMindRunTestsInput, } from "../../../contracts/index.js";
import { BaseProjectAdapter } from "../project-adapter.js";
import { MONEY_MIND_IGNORED_DIRS, resolveSafeRelativePath, } from "./money-mind-path-policy.js";
import { parseMoneyMindChapterRegistry, parseMoneyMindFeatureFlagNames, } from "./money-mind-status-parser.js";
...
export class MoneyMindProjectAdapter extends BaseProjectAdapter {
        ...
        this.operations = {
            ...
            READ_STATUS: {
                capability: {
                    ...
                    description: "Read structured V2 chapter status and feature-flag names.",
                    action: "read",
                },
            ...
            READ_FILE: {
                capability: {
                    operation: "READ_FILE",
                    description: "Read a single text file by repository-relative path (sandboxed).",
                    action: "read",
                },
            ...
            RUN_TESTS: {
                capability: {
                    operation: "RUN_TESTS",
                    description: "Run one allowlisted, existing npm script (build/lint/test/typecheck).",
                    action: "execute",
                },
            ...
            INSPECT_STRUCTURE: {
                capability: {
                    ...
                    description: "List the repository's directory structure (paths + type only).",
                    action: "read",
                },
            ...
            READ_DOCUMENTATION: {
                capability: {
                    ...
                    description: "Read (and optionally keyword-search) the project's documentation set.",
                    action: "read",
                },
    ...
    async readProject() {
        ...
        return {
            ...
            description: "Independent personal-finance web application, reached only through " +
    ...
    async readStatus() {
        const chapters = (await this.repo.exists(CHAPTER_REGISTRY_PATH))
            ? parseMoneyMindChapterRegistry(await this.repo.readTextFile(CHAPTER_REGISTRY_PATH))
            : [];
        const featureFlags = await this.readFeatureFlagNames();
        const implemented = chapters.filter((c) => c.implementationStatus === "implemented").length;
    ...
    async readTestResults() {
        ...
        return available
            ? {
                ...
                message: 'a "test" npm script is defined; call RUN_TESTS to execute it',
            }
            : {
                testsConfigured: false,
                message: 'no automated test suite (npm script "test") is defined in this project',
            };
    }
    ...
    async readFile(rawInput) {
        ...
        if (!(await this.repo.exists(safe))) {
            throw new NotFoundError(`file not found in money-mind repository: ${requested}`);
        }
    ...
    async inspectStructure(rawInput) {
        const { path: startPath, maxDepth } = validateMoneyMindInspectStructureInput(rawInput);
        const root = startPath ? resolveSafeRelativePath(startPath) : "";
        const depthLimit = Math.min(maxDepth ?? this.defaultInspectDepth, this.maxInspectDepth);
        if (!(await this.repo.exists(root))) {
            throw new NotFoundError(`directory not found in money-mind repository: ${startPath ?? "(root)"}`);
        }
        const entries = [];
        let truncated = false;
    ...
    async readDocumentation(rawInput) {
        ...
        const matched = terms.length === 0 ? scored : scored.filter((s) => s.score > 0);
    ...
    async readPackageJsonSummary() {
        ...
        try {
            ...
            throw new ValidationError(`money-mind package.json is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        if (record.scripts && typeof record.scripts === "object") {
            for (const [key, value] of Object.entries(record.scripts)) {
    ...
    async readFeatureFlagNames() {
        if (!(await this.repo.exists(FEATURE_FLAGS_PATH)))
            return [];
        return parseMoneyMindFeatureFlagNames(await this.repo.readTextFile(FEATURE_FLAGS_PATH));
    }
}

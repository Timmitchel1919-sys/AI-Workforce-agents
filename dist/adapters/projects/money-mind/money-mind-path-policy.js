...
export function isSensitiveSegment(segment) {
 ...
 */
export function resolveSafeRelativePath(relPath) {
    ...
    if (relPath.includes("\0")) {
        throw new ValidationError(`path contains an invalid character: "${relPath}"`);
    }
    const normalized = relPath.replace(/\\/g, "/").trim();
    if (normalized.startsWith("/") || /^[a-zA-Z]:/.test(normalized)) {
        throw new ValidationError(`path must be relative to the money-mind repository root: "${relPath}"`);
    }
    ...
    if (segments.some(isSensitiveSegment)) {
        throw new ValidationError(`access to this path is not permitted: "${relPath}"`);
    }
    return segments.join("/");
}
/** Directory names never descended into by structure inspection. */
export const MONEY_MIND_IGNORED_DIRS = new Set([
    "node_modules",
    "dist",
    "out",
    "coverage",

 ...
 */
import { DEFAULT_AGENT_LIMITS, ProviderUnavailableError, validateProjectManagerDecision, validateProjectManagerTask, } from "../../contracts/index.js";
...
import { PROJECT_MANAGER_AGENT_ID, PROJECT_MANAGER_AGENT_LIMITS, } from "./project-manager-agent-definition.js";
...
export class ProjectManagerAgent extends GeneralAgent {
    ...
    async run(input, task, _agent, run, _guard) {
        ...
        if (!parsed) {
            throw this.fail("model_failure", `${input.mode} model returned no parseable JSON`);
        }
    ...
    async callModel(task, run, input) {
        ...
        const system = input.mode === "decompose"
        ...
        const user = input.mode === "decompose"
            ? [
            ...
            : [
                `Objective: ${input.objective}`,
                "Prior results:",
                ...input.priorResults.map((r) => `- ${r.specId} (${r.agentId ?? "unassigned"}) [${r.status}]: ${r.summary}`),
            ].join("\n");
...
function normalizeSubtasks(value) {
    ...
    return value
        ...
        recommendedAgentId: typeof s.recommendedAgentId === "string"
            ? s.recommendedAgentId
            : undefined,
        recommendedCapability: typeof s.recommendedCapability === "string"

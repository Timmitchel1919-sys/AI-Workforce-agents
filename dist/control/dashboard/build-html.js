...
`;
function nav() {
    return DASHBOARD_VIEWS.map((view, i) => `<button data-nav="${view}"${i === 0 ? ' aria-current="true"' : ""}>${escapeHtml(view[0].toUpperCase() + view.slice(1))}</button>`).join("");
}
function clientScript(commandEndpoint) {
...
export function buildDashboardHtml(snapshot, options = {}) {

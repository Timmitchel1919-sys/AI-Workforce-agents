...
/* ------------------------------------------------------------------ */
export function renderOverview(status, recent) {
...
export function renderAgents(agents) {
    if (agents.length === 0)
        return empty("No agents registered.");
...
export function renderWorkflows(workflows) {
    if (workflows.length === 0)
        return empty("No workflows yet.");
    return workflows
        .map((w) => {
        const pct = w.progress.total === 0
            ? 0
            : Math.round((w.progress.completed / w.progress.total) * 100);
        const stages = w.stages
            .map((s) => `<li class="stage stage--${esc(s.status)}">${esc(s.type)} <small>${esc(s.status)}</small>${s.error ? `<div class="stage__err">${esc(s.error)}</div>` : ""}</li>`)
            .join("");
        return `
        <article class="wf">
...
export function renderTasks(tasks, filter = {}) {
    let shown = tasks;
    if (filter.status)
        shown = shown.filter((t) => t.status === filter.status);
    if (filter.projectId)
        shown = shown.filter((t) => t.projectId === filter.projectId);
    if (shown.length === 0)
        return empty("No tasks match the current filter.");
...
export function renderApprovals(approvals) {
    ...
    const parts = [];
    if (pending.length > 0) {
        parts.push(`<h3>Pending (${pending.length})</h3>`, pending.map((a) => card(a, true)).join(""));
    }
    else {
    ...
    if (decided.length > 0) {
        parts.push(`<h3>Recent decisions</h3>`, decided
            .slice(0, 20)
            .map((a) => card(a, false))
            .join(""));
    }
...
export function renderAuditRows(events) {
    if (events.length === 0)
        return empty("No audit events.");
    const rows = events.map((e) => [
        esc(e.timestamp),
        esc(e.type),
        e.actor ? esc(e.actor) : "—",
        esc([e.projectId, e.workflowId, e.taskId, e.toolId]
            .filter(Boolean)
            .join(" / ") || "—"),
...
export function renderSnapshotBody(snapshot) {
    ...
    const banner = snapshot.error
        ...
        : "";
    return (banner +
        DASHBOARD_VIEWS.map((view, i) => `<section id="view-${view}" data-view="${view}"${i === 0 ? "" : " hidden"}>

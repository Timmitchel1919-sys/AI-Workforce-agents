...
import { capabilitiesSatisfied, satisfiesToolchainRequirements, } from "./capability-mapping.js";
export class EnvironmentRouter {
    ...
    route(requirement) {
        ...
        const hostIds = new Set(this.registry
            .listHosts()
        ...
        const candidates = this.registry
            ...
            .filter((instance) => requirement.descriptorId === undefined ||
            instance.descriptorId === requirement.descriptorId)
            .filter((instance) => requirement.environmentType === undefined ||
            instance.environmentType === requirement.environmentType)
            .filter((instance) => capabilitiesSatisfied(requirement.requiredCapabilities ?? [], instance.capabilities))
            .filter((instance) => satisfiesToolchainRequirements(requirement.toolchains ?? [], instance.toolchains))
        ...
        const provisionableDescriptorId = requirement.descriptorId ??
        ...
        if (provisionableDescriptorId !== undefined) {
            return {
                ...
                reason: `descriptor "${provisionableDescriptorId}" is supported but no ` +
...
function firstOfferingCapability(registry, requirement) {

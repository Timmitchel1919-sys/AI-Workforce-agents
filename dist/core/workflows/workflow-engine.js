 ...
 * blindly.
 */
import { AgentExecutionError, NotFoundError, StateTransitionError, ValidationError, validateProjectManagerDecision, } from "../../contracts/index.js";
import { assignAgent, computeReadySpecs, extractFailureReason, extractToolCallCount, hasActionableWork, shouldRetry, } from "./workflow-graph.js";
export class WorkflowEngine {
    ...
    async run(workflowId) {
        ...
        if (workflow.status !== "running") {
            throw new StateTransitionError(`workflow ${workflowId} is not runnable from status ${workflow.status}`);
        }
    ...
    async resume(workflowId) {
        ...
        if (workflow.status !== "awaiting_approval") {
            throw new StateTransitionError(`workflow ${workflowId} is not awaiting approval (status: ${workflow.status})`);
        }
        const parked = workflow.taskRecords.filter((r) => r.status === "awaiting_approval");
        workflow = this.workflows.transition(workflow.id, "running");
     ...
     */
    async planFromObjective(input, depth = 0) {
        const maxDepth = input.limits?.maxDelegationDepth ?? 1;
        if (depth >= maxDepth) {
            throw new ValidationError(`planFromObjective: delegation depth ${depth} would exceed maxDelegationDepth (${maxDepth})`);
        }
        if (!this.registry.has(this.projectManagerAgentId)) {
            throw new NotFoundError(`project manager agent "${this.projectManagerAgentId}" is not registered`);
        }
        ...
        if (planningTask.status !== "completed") {
            throw new AgentExecutionError(this.projectManagerAgentId, "internal_error", `planning task did not complete (status: ${planningTask.status})`, { taskId: planningTask.id });
        }
        const decision = planningTask.output;
        validateProjectManagerDecision(decision);
        const plan = decision;
        const tasks = plan.subtasks.map((sub) => ({
            id: sub.id,
            type: sub.type,
    ...
    /* scheduling                                                     */
    /* -------------------------------------------------------------- */
    async schedule(workflow, startedAtMs) {
        ...
        const maxPasses = workflow.tasks.length * (workflow.retryPolicy.maxRetries + 1) + 5;
        for (let pass = 0; pass < maxPasses; pass++) {
            ...
            if (ready.length === 0) {
                ...
                if (workflow.taskRecords.some((r) => r.status === "awaiting_approval")) {
                    return this.workflows.transition(workflow.id, "awaiting_approval");
                }
                return this.finalize(workflow);
            }
            for (const spec of ready) {
                if (workflow.counters.agentExecutions >=
                    workflow.limits.maxAgentExecutions) {
                    return this.fail(workflow, "limit_exceeded: maxAgentExecutions");
                }
                const assignment = assignAgent(spec, workflow, this.registry, this.permissions, this.toolRegistry);
                ...
                if (!assignment.validated) {
                    ...
                    if (workflow.failureBehavior === "abort") {
                        return this.fail(workflow, `assignment invalid for "${spec.id}": ${assignment.reason}`);
                    }
                    continue;
                }
    ...
    async dispatch(workflow, spec, agentId) {
        ...
        workflow = this.workflows.incrementCounter(workflow.id, "toolCalls", extractToolCallCount(task.output));
    ...
    applyTaskOutcome(workflow, specId, task) {
    ...
    /* handoffs                                                       */
    /* -------------------------------------------------------------- */
    async ensureHandoffs(workflow, spec, destinationAgentId) {
        for (const depId of spec.dependsOn) {
            ...
            const alreadyHandedOff = this.handoffs
                ...
                .some((h) => h.destinationAgentId === destinationAgentId &&
    ...
    /* completion / failure                                           */
    /* -------------------------------------------------------------- */
    propagateSkip(workflow, failedSpecId) {
        ...
        while (changed) {
            ...
            for (const spec of workflow.tasks) {
                ...
                if (record.status !== "pending" || toSkip.has(spec.id))
                    continue;
                const blocked = spec.dependsOn.some((dep) => dep === failedSpecId || toSkip.has(dep));
                if (blocked) {
                    toSkip.add(spec.id);
                    changed = true;
                }
    ...
    finalize(workflow) {
        const completed = workflow.taskRecords.filter((r) => r.status === "completed");
        const failed = workflow.taskRecords.filter((r) => r.status === "failed" ||
            r.status === "blocked" ||
            r.status === "skipped");
        const allCompleted = completed.length === workflow.tasks.length;
        const taskResults = workflow.taskRecords.map((r) => ({
            specId: r.specId,
            status: r.status,
        ...
        if (failed.length > 0) {
            ...
            workflow = this.workflows.setResult(workflow.id, result);
            return this.fail(workflow, `${failed.length} task(s) failed or were blocked`);
        }
        return this.fail(workflow, "no tasks completed");
    }
    emit(workflow, kind, data) {

...
export function computeReadySpecs(workflow) {
    const recordBySpec = new Map(workflow.taskRecords.map((record) => [record.specId, record]));
    return workflow.tasks.filter((spec) => {
        const record = recordBySpec.get(spec.id);
        if (!record || record.status !== "pending")
            return false;
        return spec.dependsOn.every((dep) => recordBySpec.get(dep)?.status === "completed");
    });
}
/** True while any task could still run or is waiting on something. */
export function hasActionableWork(workflow) {
    return workflow.taskRecords.some((record) => ["pending", "ready", "dispatched", "awaiting_approval"].includes(record.status));
}
/**
 ...
 * when supplied.
 */
export function assignAgent(spec, workflow, registry, permissions, toolRegistry) {
    const candidateId = spec.agentId ??
        registry
            .eligible(spec.type, workflow.projectId)
            .find((agent) => workflow.participatingAgents.includes(agent.id) &&
            (!spec.capability || agent.capabilities.includes(spec.capability)))?.id;
    if (!candidateId) {
        return {
            specId: spec.id,
            validated: false,
    ...
    return {
        ...
        agentId: candidateId,
        validated: true,
        reason: permissionNotes.length > 0
 ...
 */
export function extractFailureReason(message) {
    const match = /^\[[^:]+:([\w-]+)]/.exec(message);
    return match?.[1];
}
export function shouldRetry(policy, reason, retryCountSoFar) {
    if (retryCountSoFar >= policy.maxRetries)
        return false;
    if (!reason)
        return false;
...
export function extractToolCallCount(output) {
    ...
    if (!counters || typeof counters !== "object")
        return 0;
    const toolCalls = counters.toolCalls;
    return typeof toolCalls === "number" ? toolCalls : 0;
}
export function initialTaskRecord(specId, now) {
    return {
        specId,
        status: "pending",
        retryCount: 0,

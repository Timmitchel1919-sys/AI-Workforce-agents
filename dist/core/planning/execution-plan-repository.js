...
export class ExecutionPlanRepository {
    ...
    create(plan) {
        ...
        if (latest && plan.version !== latest.version + 1) {
            throw new ValidationError(`execution plan ${plan.planId} must continue at version ${latest.version + 1}`);
        }
    ...
    transition(next) {
        ...
        for (const key of Object.keys({
            ...
            if (JSON.stringify(current[key]) !== JSON.stringify(next[key])) {
                throw new ValidationError(`execution plan field "${key}" is immutable once created`);
            }
        }
    ...
    require(id) {
        ...
        if (!plan)
            throw new NotFoundError(`unknown execution plan: ${id}`);
        return plan;
    }

 ...
 */
import { AGENT_REJECTION_REASONS, BLOCKER_CODES, EXECUTION_PLAN_SCHEMA_VERSION, PLAN_STATUSES, ValidationError, validateExecutionPlan, } from "../../contracts/index.js";
...
export function serializeExecutionPlan(plan) {
    ...
    const record = sortKeys(JSON.parse(JSON.stringify(plan)));
...
export function deserializeExecutionPlan(record) {
    const id = record && typeof record === "object" && "id" in record
        ? String(record.id)
        : undefined;
    try {
...
export function assertPlanIntegrity(plan) {
    ...
    for (const env of plan.environments) {
        ...
        if ((env.status === "satisfied") !==
            Boolean(env.match.selectedInstanceId)) {
            throw new ValidationError(`environment ${env.id} selection and status disagree`);
        }
    }
    ...
    for (const assignment of plan.agents) {
        ...
        if ((assignment.qualification === "qualified") !==
            ...
            throw new ValidationError("agent assignment qualification is inconsistent");
        }
    ...
    for (const model of plan.models) {
        if (!agentReqIds.has(model.agentRequirementId)) {
            throw new ValidationError("model requirement references no agent requirement");
        }
    }
    const buildIds = uniqueIds(plan.build, "build");
    for (const stage of plan.build) {
        if (!envIds.has(stage.environmentRequirementId)) {
            throw new ValidationError(`build stage ${stage.id} references no environment`);
        }
        if (!agentReqIds.has(stage.agentRequirementId)) {
            throw new ValidationError(`build stage ${stage.id} references no agent requirement`);
        }
    }
    uniqueIds(plan.tests, "tests");
    for (const stage of plan.tests) {
        if (!stage.dependsOnStageIds.every((id) => buildIds.has(id))) {
            throw new ValidationError(`test stage ${stage.id} depends on an unknown stage`);
        }
    }
...
function uniqueIds(items, field) {
    ...
    for (const item of items) {
        ...
        if (ids.has(item.id)) {
            throw new ValidationError(`plan.${field} contains duplicate id ${item.id}`);
        }
        ids.add(item.id);
    }
    return ids;

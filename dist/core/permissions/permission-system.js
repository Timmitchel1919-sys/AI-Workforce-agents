import { PermissionDeniedError, } from "../../contracts/index.js";
/**
...
export class PermissionSystem {
    ...
    evaluate(request) {
        const matches = this.grants.filter((grant) => grant.action === request.action && this.scopeMatches(grant, request));
    ...
    assert(request) {
        ...
        if (!decision.allowed) {
            ...
            throw new PermissionDeniedError(`permission denied: ${request.agentId} ${request.action} on ` +
                `${request.projectId}${tool} (${request.environment}) — ` +
                decision.reason);
        }
    }
    scopeMatches(grant, request) {
        return ((grant.agentId === undefined || grant.agentId === request.agentId) &&

...
export class TechnologySelector {
    ...
    select(requirement) {
        ...
        if (requirement.environmentType !== undefined) {
            const descriptor = this.registry.descriptorForType(requirement.environmentType);
            if (descriptor)
                return { outcome: "SUPPORTED", descriptor };
        }
        if ((requirement.requiredCapabilities ?? []).length > 0) {
            const offering = this.bestOfferingForCapabilities(requirement.requiredCapabilities ?? []);
            if (offering)
                return { outcome: "SUPPORTED", descriptor: offering };
        }
        if ((requirement.toolchains ?? []).length > 0) {
            const supporting = this.registry
                .listDescriptors()
                .find((descriptor) => requirement.toolchains.every((req) => descriptor.supportedToolchains.some((supported) => supported.kind === req.kind)));
            if (supporting)
                return { outcome: "SUPPORTED", descriptor: supporting };
        }
        return {
            outcome: "UNSUPPORTED",
    ...
    bestOfferingForCapabilities(capabilities) {
        let best;
        let bestCount = 0;

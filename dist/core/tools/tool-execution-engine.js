import { NotFoundError, ValidationError, toolPermissionRequest, validateToolExecutionRequest, } from "../../contracts/index.js";
import { ApprovalSystem } from "../approvals/approval-system.js";
import { createId, now } from "../shared.js";
import { agentAllowed, environmentAllowed, projectAllowed, toolApprovalRequired, } from "./tool-policy.js";
...
export class ToolExecutionEngine {
    ...
    createRequest(draft) {
        const fallbackAction = this.registry.get(draft.toolId)?.requiredPermission.action ?? "execute";
        return {
    ...
    async execute(request) {
        ...
        try {
            ...
            return this.finish(request, started, "failure", this.err("invalid_request", message(error)));
        }
        this.emit(request, "validated", {});
        const tool = this.registry.get(request.toolId);
        if (!tool) {
            return this.finish(request, started, "failure", this.err("unknown_tool", `no tool registered as "${request.toolId}"`));
        }
        this.emit(request, "resolved", { version: tool.version });
        if (!agentAllowed(tool, request.agentId)) {
            return this.deny(request, started, "agent_not_allowed", `agent "${request.agentId}" may not use tool "${tool.id}"`);
        }
        if (!projectAllowed(tool, request.projectId)) {
            return this.deny(request, started, "project_not_allowed", `project "${request.projectId}" may not use tool "${tool.id}"`);
        }
        if (!environmentAllowed(tool, request.environment)) {
            return this.deny(request, started, "environment_not_allowed", `tool "${tool.id}" is not allowed in environment "${request.environment}"`);
        }
        ...
        if (this.count(this.taskCalls, taskKey(request)) >= limits.maxCallsPerTask) {
            return this.limitExceeded(request, started, "call_limit_exceeded", {
                scope: "task",
                max: limits.maxCallsPerTask,
            });
        }
        if (this.count(this.agentCalls, agentKey(request)) >= limits.maxCallsPerAgent) {
            return this.limitExceeded(request, started, "call_limit_exceeded", {
                scope: "agent",
     ...
     */
    async resume(requestId, options = {}) {
        const held = this.pending.get(requestId);
        if (!held) {
            throw new NotFoundError(`no tool request awaiting approval: ${requestId}`);
        }
        if (options.asOf)
            this.approvals.expireStale(options.asOf);
        const approval = this.approvals.require(held.approvalId);
        if (approval.status === "requested") {
            throw new ValidationError(`approval ${approval.id} for request ${requestId} is still pending`);
        }
        const started = this.clock();
        ...
        if (approval.status === "rejected") {
            ...
            return this.finish(held.request, started, "denied", this.err("approval_rejected", "the approval was rejected", {
                approvalId: approval.id,
            }));
        }
        if (approval.status === "expired") {
            ...
            return this.finish(held.request, started, "denied", this.err("approval_expired", "the approval expired", {
                approvalId: approval.id,
            }));
        }
        // approved — re-authorize (grants may have changed) then execute
        const tool = this.registry.require(held.request.toolId);
        const decision = this.permissions.evaluate(toolPermissionRequest(held.request));
        if (!decision.allowed) {
            return this.deny(held.request, started, "permission_denied", decision.reason);
        }
    ...
    /* internals                                                      */
    /* -------------------------------------------------------------- */
    async runTool(request, tool, started) {
        const limits = this.effectiveLimits(tool);
        ...
        if (tool.inputSchema) {
            try {
                ...
                return this.finish(request, started, "failure", this.err("malformed_result", `input schema rejected the request: ${message(error)}`));
            }
        }
        ...
        let output;
        try {
            output = await withTimeout(() => tool.execute(request.input, context), timeoutMs);
        }
        catch (error) {
            if (error instanceof ToolTimeoutSignal) {
                this.emit(request, "timeout", { timeoutMs });
                return this.finish(request, started, "timeout", this.err("timeout", `tool timed out after ${timeoutMs}ms`, {
                    timeoutMs,
                }));
            }
            this.emit(request, "failed", { error: message(error) });
            return this.finish(request, started, "failure", this.err("tool_error", message(error)));
        }
        const elapsed = this.clock() - callStart;
        if (elapsed > timeoutMs) {
            this.emit(request, "timeout", { timeoutMs, elapsed });
            return this.finish(request, started, "timeout", this.err("timeout", `tool exceeded ${timeoutMs}ms (took ${elapsed}ms)`, { timeoutMs, elapsed }));
        }
        ...
        if (outputBytes > limits.maxOutputBytes) {
            ...
            return this.finish(request, started, "failure", this.err("output_too_large", `output ${outputBytes} bytes exceeds ${limits.maxOutputBytes}`, { outputBytes, max: limits.maxOutputBytes }));
        }
        if (tool.outputSchema) {
            try {
                ...
                this.emit(request, "failed", { reason: "malformed_result" });
                return this.finish(request, started, "failure", this.err("malformed_result", `output schema rejected the result: ${message(error)}`));
            }
        }
    ...
    effectiveLimits(tool) {
        const t = tool.limits;
        const c = this.ceiling;
        return {
            maxCallsPerTask: Math.min(t.maxCallsPerTask, c.maxCallsPerTask ?? t.maxCallsPerTask),
            maxCallsPerAgent: Math.min(t.maxCallsPerAgent, c.maxCallsPerAgent ?? t.maxCallsPerAgent),
            maxDurationMs: Math.min(t.maxDurationMs, c.maxDurationMs ?? t.maxDurationMs),
            maxInputBytes: Math.min(t.maxInputBytes, c.maxInputBytes ?? t.maxInputBytes),
            maxOutputBytes: Math.min(t.maxOutputBytes, c.maxOutputBytes ?? t.maxOutputBytes),
        };
    }
    count(map, key) {
    ...
    err(reason, message, details = {}) {
        return { reason, message, details };
    }
    deny(request, started, reason, message) {
        this.emit(request, "denied", { reason, message });
        return this.finish(request, started, "denied", this.err(reason, message));
    }
    limitExceeded(request, started, reason, details) {
        this.emit(request, "limit_exceeded", { reason, ...details });
        return this.finish(request, started, "failure", this.err(reason, `execution limit exceeded (${reason})`, details));
    }
    finish(request, started, status, error) {
        return {
            requestId: request.requestId,
            toolId: request.toolId,
            taskId: request.taskId,

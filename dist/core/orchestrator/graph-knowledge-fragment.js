...
export class KnowledgeSourceRegistry {
    ...
    register(source) {
        ...
        if (!KNOWLEDGE_SOURCE_KINDS.includes(source.kind)) {
            throw new Error(`unsupported knowledge source kind: ${source.kind}`);
        }
        if (!source.title.trim())
            throw new Error("knowledge source needs a title");
 ...
 */
export function buildKnowledgeFragment(input) {
    const visible = (input.provider?.list() ?? [])
        .filter((s) => s.projectIds.includes(input.projectId) || s.projectIds.includes("*"))
    ...
    for (const s of visible) {
        ...
        for (const ref of s.referencesSourceIds ?? []) {
            if (!visibleIds.has(ref) || ref === s.id)
                continue;
            edges.push({

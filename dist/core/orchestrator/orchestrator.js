...
export const selectFirstById = (candidates) => [...candidates].sort((a, b) => a.id.localeCompare(b.id))[0];
/** Default approval policy: nothing needs approval. */
export const approveNothing = {
...
export class Orchestrator {
    ...
    environment;
    agentGate;
    constructor(registry, tasks, handoffs, audit, executor, approvals = new ApprovalSystem(), options = {}) {
     ...
     */
    recordApprovalDecision(approvalId, decision, decidedBy, metadata = {}) {
        const updated = this.approvals.decide(approvalId, decision, decidedBy, metadata);
        const linkedTaskId = typeof updated.decisionMetadata.taskId === "string"
    ...
    async resume(taskId, options = {}) {
        ...
        if (task.status !== "awaiting_approval") {
            throw new StateTransitionError(`task ${taskId} is not awaiting approval (status: ${task.status})`);
        }
        if (!task.approvalId) {
        ...
        if (approval.status === "requested") {
            throw new WorkforceError(`cannot resume task ${taskId}: approval ${approval.id} is still pending`);
        }
        ...
        if (outcome !== "approved") {
            const reason = outcome === "rejected" ? "approval rejected" : "approval expired";
    ...
    requestHandoff(draft) {
        if (!this.registry.has(draft.sourceAgentId) ||
     ...
     */
    enforcePermissions(task, agent) {
        if (task.requiredPermissions.length === 0)
            return undefined;
        if (!this.permissions) {
            return new PermissionDeniedError(`permission denied: no permission system configured for gated task ${task.id}`);
        }
        for (const required of task.requiredPermissions) {
            ...
            if (!decision.allowed) {
                ...
                return new PermissionDeniedError(`permission denied: ${agent.id} ${required.action} on ` +
                    `${task.projectId}${tool} (${this.environment}) — ${decision.reason}`);
            }
        }
        return undefined;
    }
    makeGuard(task, agent) {
        const permissions = this.permissions;
        const environment = this.environment;
        return {
            assert(action, toolId) {
                if (!permissions)
                    return;
                permissions.assert({
                    action,
    ...
    toPermissionRequest(task, agent, required) {
        return {
            action: required.action,
            toolId: required.toolId,
            agentId: agent.id,
    ...
    failTask(task, agent, message, reason) {
        const failed = this.tasks.fail(task.id, message);
        this.audit.record("task_failed", {
            taskId: failed.id,
            agentId: agent.id,

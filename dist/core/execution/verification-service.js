 ...
 */
import { NotFoundError, PermissionDeniedError, StateTransitionError, ValidationError, isTerminalSession, operationWorkspaceAccess, operatorCan, operatorCanAccessProject, requireExecutionId, validateVerificationProfile, } from "../../contracts/index.js";
...
export class VerificationService {
    ...
    registerProfile(profile) {
        ...
        for (const stage of Object.values(profile.stages)) {
            if (!this.options.operations.get(stage.operationId)) {
                throw new ValidationError(`profile operation ${stage.operationId} is not registered`);
            }
        }
     ...
     */
    async start(principal, raw, idempotencyKey) {
        ...
        if (existingId) {
            ...
            if (existing.result.projectId !== request.projectId ||
                existing.result.plan.planId !== request.planId ||
                existing.result.plan.version !== request.planVersion) {
                throw new StateTransitionError("idempotency key was already used for a different verification");
            }
        ...
        const running = [...this.runs.values()].filter((r) => r.result.projectId === request.projectId &&
            (r.result.status === "running" || r.result.status === "pending")).length;
        if (running >= (this.options.maxConcurrentPerProject ?? 1)) {
            throw new StateTransitionError("the project already has the maximum number of running verifications");
        }
        const planned = [
            ...plan.build.map((b) => ({
                stageId: b.id,
        ...
        if (request.sourceSessionId) {
            const source = await this.options.manager.getSession(principal, request.sourceSessionId);
            if (source.projectId !== request.projectId) {
                throw new NotFoundError("resource not found");
            }
            if (!isTerminalSession(source.status)) {
                throw new StateTransitionError("complete the developer session before verifying its ChangeSet");
            }
            const cs = await this.options.manager.getChangeSet(principal, request.sourceSessionId);
            if (cs)
                changeSet = {
    ...
    validateStart(raw) {
        ...
        return {
            ...
            ...(value.sourceSessionId !== undefined
                ? {
                    sourceSessionId: requireExecutionId(value.sourceSessionId, "sourceSessionId"),
                }
                : {}),
        };
    }
     ...
     */
    validateGraph(planned, profile) {
        const ids = new Set(planned.map((p) => p.stageId));
        for (const stageId of Object.keys(profile.stages)) {
            if (!ids.has(stageId)) {
                throw new ValidationError(`profile stage ${stageId} is not in the plan revision`);
            }
        }
        const deps = new Map();
        for (const p of planned) {
            ...
            for (const d of all) {
                if (!ids.has(d)) {
                    throw new ValidationError(`stage ${p.stageId} depends on unknown stage ${d}`);
                }
            }
        ...
        const visit = (id, path) => {
            ...
            if (mark.get(id) === "visiting") {
                throw new ValidationError(`stage dependency cycle: ${[...path, id].join(" -> ")}`);
            }
    ...
    /* Run                                                            */
    /* -------------------------------------------------------------- */
    setStage(state, stageId, patch) {
        const r = state.result;
        state.result = {
            ...r,
            stages: r.stages.map((s) => s.stageId === stageId ? { ...s, ...patch } : s),
        };
    }
    stage(state, stageId) {
        return state.result.stages.find((s) => s.stageId === stageId);
    }
    async run(state, planned, graph, profile, changeSet) {
        ...
        while (pending.length > 0 || running.size > 0) {
            ...
            for (const id of [...pending]) {
                ...
                if (blocking.some((s) => s.status === "running" || pending.includes(s.stageId))) {
                    continue;
                }
                ...
                if (failedDep || stopReason) {
                    ...
                    progressed = true;
                    this.setStage(state, id, failedDep
                        ? {
                            failure: {
        ...
        if (changeSet) {
            control.setChangeSetStatus?.(changeSet.workspaceId, status === "passed" ? "verified" : "verification_failed");
        }
        return finished;
    }
    overallStatus(state, profile, reasons) {
        if (state.cancelRequested)
            return "cancelled";
        if (reasons.some((r) => r.code === "SOURCE_CHANGED"))
            return "failed";
        ...
        if (profile.requireAllPlannedStages &&
    ...
    finish(state, status, reasons) {
        ...
        state.result = result;
        this.record("verification_completed", state.principal.id, result.projectId, {
    ...
    async runStage(state, stageId, stageProfile, changeSet) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            ...
            let session;
            try {
                ({ session } = await this.options.manager.createSession(principal, {
            ...
            this.observeEnvironment(state, session.environmentInstanceId, session.sandbox?.providerId);
            ...
            try {
                ...
                patch = {
                    ...
                    failure: state.cancelRequested
                        ...
                        : {
                            kind: "EXECUTION_ERROR",
                            detail: error instanceof Error ? error.message : "invocation failed",
                        },
                };
                break;
            }
            ...
            const transient = invocation.exitClass === "sandbox_failure" ||
                invocation.exitClass === "resource_limit";
            if (!transient || state.cancelRequested)
                break;
        }
    ...
    classify(op, invocation) {
        switch (invocation.exitClass) {
            ...
            case "tool_failure": {
                ...
                const invalid = invocation.reasons.find((r) => r.code === "INVALID_OUTPUT");
                return {
                    status: "failed",
                    failure: {
            ...
            default:
                return {
                    ...
                    failure: {
                        kind: "EXECUTION_ERROR",
                        detail: invocation.reasons[0]?.detail ?? "the sandbox reported a failure",
                    },
                };
        }
    }
    denialFailure(reasons) {
        for (const kind of DENIAL_KINDS) {
            const reason = reasons.find((r) => r.code === kind);
            if (reason)
                return { kind, detail: reason.detail };
        }
        return {
            kind: "POLICY_DENIED",
            detail: reasons.map((r) => `${r.code}: ${r.detail}`).join("; ") ||
    ...
    findings(invocation) {
        try {
            const parsed = JSON.parse(invocation.output?.text ?? "");
    ...
    observeEnvironment(state, environmentInstanceId, providerId) {
        ...
        if (!next.environmentInstanceId && environmentInstanceId) {
            ...
            const toolchains = (instance?.toolchains ?? []).map((t) => ({
                kind: t.kind,
                name: t.name,
                ...(t.version
                    ? {
    ...
    /* Cancel / read                                                  */
    /* -------------------------------------------------------------- */
    async cancel(principal, verificationId, reason) {
        ...
        if (state.result.completedAt)
            return state.result;
        state.cancelRequested ??= { by: principal.id, reason };
        this.record("verification_cancel_requested", principal.id, state.result.projectId, {
            verification: verificationId,
        });
        await Promise.all([...state.activeSessions].map((id) => this.options.manager
            .cancel(principal, id, reason, "cancel")
            .catch(() => undefined)));
        return state.result;
    }
    get(principal, verificationId) {
    ...
    scoped(principal, verificationId, capability) {
        const state = this.runs.get(requireExecutionId(verificationId, "verificationId"));
        if (!state)
            throw new NotFoundError("resource not found");

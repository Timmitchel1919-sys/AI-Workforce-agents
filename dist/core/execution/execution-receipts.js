...
export function boundOutput(text, maxBytes, knownSecrets = []) {
...
export function createLogEntry(input) {
    const bounded = boundOutput(input.message, input.maxBytes ?? 2048, input.knownSecrets);
 ...
 */
export function createExecutionReceipt(input, knownSecrets = []) {
    ...
    const clean = scrub(structuredClone(input));
...
export class InMemoryExecutionReceiptStore {
    ...
    record(receipt) {
        if (this.receipts.has(receipt.receiptId)) {
            throw new ValidationError(`receipt ${receipt.receiptId} already exists; receipts are immutable`);
        }

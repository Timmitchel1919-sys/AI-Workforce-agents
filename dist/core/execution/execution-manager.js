...
import { evaluatePolicy, } from "./execution-policy.js";
...
export class ExecutionManager {
     ...
     * existence leak).
     */
    async preflight(principal, rawRequest) {
    ...
    async evaluate(principal, request, capability, options = {}) {
        ...
        const environmentInstanceId = this.revalidateEnvironment(plan, stage, evaluation?.rule?.requiredEnvironmentCapabilities ?? [], fail);
            ...
            ...(decision === "ELIGIBLE" &&
                ...
                ? {
                    internal: {
                        ...
                        approvalIds: requiredApprovals.flatMap((a) => a.approvalId ? [a.approvalId] : []),
                    },
                }
                : {}),
        };
    }
    resolveOperation(request, kind, fail) {
    ...
    revalidateAgent(plan, stage, fail) {
        ...
        const assignment = plan.agents.find((a) => a.requirementId === requirement.id);
        ...
        const [evidence] = this.qualification.evaluateCandidates([agent], {
    ...
    revalidateEnvironment(plan, stage, requiredCapabilities, fail) {
        ...
        try {
            ...
            const candidate = evidence.candidates.find((c) => c.instanceId === instanceId);
        ...
        const missing = requiredCapabilities.filter((c) => !instance.capabilities.some((d) => d.capability === c && d.available));
        if (missing.length > 0) {
            fail("environment", {
                code: "ENVIRONMENT_UNAVAILABLE",
    ...
    revalidateApprovals(plan, stage, request, evaluation, fail) {
        const needed = plan.approvalRequirements
            .filter((r) => r.subjectIds.includes(request.stageId))
            .map((r) => r.reason);
        if (evaluation?.requiresApproval)
            needed.push("operation_risk");
        if (needed.length === 0)
            return [];
        ...
        const valid = plan.approval.state === "approved" &&
            approval !== undefined &&
     ...
     */
    async createSession(principal, rawRequest, idempotencyKey) {
        const key = requireExecutionId(idempotencyKey, "idempotencyKey");
        const request = validateExecutionRequest(rawRequest);
        const existing = await this.options.sessions.findByIdempotencyKey(principal.id, key);
        if (existing) {
            if (existing.projectId !== request.projectId ||
                !sameRequest(existing, request)) {
                throw new StateTransitionError("idempotency key was already used for a different request");
            }
        ...
        if ((await this.options.sessions.commit(session)) === "conflict") {
            ...
            throw new StateTransitionError("execution session could not be created (conflict)");
        }
    ...
    listOperations(principal) {
        if (!operatorCan(principal, "view")) {
            throw new PermissionDeniedError(`operator ${principal.id} lacks the view capability`);
        }
    ...
    async listSessions(principal, projectId) {
    ...
    async reconcile(session, actor) {
        ...
        const budgetExhausted = now >= Date.parse(session.createdAt) + session.limits.sessionTimeoutMs;
        const open = session.attempts.find((a) => a.status === "running" || a.status === "pending");
        const orphaned = open?.startedAt !== undefined &&
            now >
                Date.parse(open.startedAt) +
                    session.limits.operationTimeoutMs +
                    ORPHAN_GRACE_MS;
        if (!budgetExhausted && !orphaned)
            return session;
        const at = this.clock();
        let next = session;
        if (open) {
            next = finishAttempt(next, open.attemptId, orphaned && !budgetExhausted ? "failed" : "timed_out", at);
        }
        const reason = budgetExhausted
            ? {
                code: "TIMEOUT",
                detail: "the session time budget was exhausted without a live invocation",
            }
            : {
                code: "SANDBOX_FAILURE",
                detail: "interrupted: the invocation's owner is gone (orphaned); outcome unknown, not successful",
            };
        ...
        if ((await this.options.sessions.commit(next, session.revision)) !==
            "committed") {
     ...
     */
    async cancel(principal, sessionId, reason, kind) {
        if (typeof reason !== "string" ||
            reason.trim().length === 0 ||
            reason.length > 500) {
            throw new ValidationError("a reason (1-500 characters) is required");
        }
        const stored = await this.options.sessions.get(requireExecutionId(sessionId, "sessionId"));
        if (!stored)
            throw new NotFoundError("resource not found");
        if (!operatorCanAccessProject(principal, stored.projectId) ||
        ...
        if (!operatorCanAccessProject(principal, current.projectId) ||
            !this.options.projects.has(current.projectId)) {
            throw new NotFoundError("resource not found");
        }
        this.authorize(principal, current.projectId, kind === "kill" ? "kill_execution" : "cancel_execution");
        for (let tries = 0; tries < 3; tries += 1) {
            const session = tries === 0 ? current : await this.options.sessions.get(sessionId);
            if (!session)
                throw new NotFoundError("resource not found");
            ...
            if ((await this.options.sessions.commit(planned.session, session.revision)) === "committed") {
                this.record(kind === "kill" ? "kill_requested" : "cancel_requested", principal.id, session.projectId, {
                    execution: session.sessionId,
                    outcome: planned.outcome,
...
function stageOf(plan, stageId) {
    const requirementFor = (purpose, componentIds) => plan.agentRequirements.find((r) => r.purpose === purpose &&
        componentIds.some((c) => r.componentIds.includes(c)))?.id;
    ...
    if (test) {
        return {
            ...
            agentRequirementId: requirementFor("test", [test.componentId]) ??

...
const CAPABILITIES = [
  { icon: InfinityIcon, title: "splash.capabilities.multiAgent", subtitle: "splash.capabilities.orchestration" },
  { icon: ShieldCheck, title: "splash.capabilities.enterprise", subtitle: "splash.capabilities.security" },
  { icon: Server, title: "splash.capabilities.environment", subtitle: "splash.capabilities.flexibility" },
  { icon: LineChart, title: "splash.capabilities.business", subtitle: "splash.capabilities.impact" },
] as const;
...
export function BrandedSplash({
  ...
  const message = t(finished && progress < 100 ? "splash.messages.starting" : init.message);
    ...
    >
      ...
      <header className="splash-top">
        <div className="splash-brand">
          ...
          <span className="splash-brand__tagline">{t("common.tagline")}</span>
        </div>
      ...
      <aside className="splash-side splash-side--left" aria-hidden="true">
        {(["people", "ideas", "agents", "technology", "impact"] as const).map((item) => (
          <span key={item}>{t(`splash.side.${item}`)}</span>
        ))}
      </aside>
      <aside className="splash-side splash-side--right" aria-hidden="true">
        <span className="splash-side__pair">
          {t("splash.side.global")}
          <br />
          {t("splash.side.local")}
        </span>
        <span className="splash-side__quote">
          {t("splash.side.quote1")}
          <br />
          {t("splash.side.quote2")}
        </span>
      </aside>
      ...
      <main className="splash-center">
        ...
        <h1 className="splash-title">{t("common.brand")}</h1>
        <p className="splash-tagline">{t("common.tagline")}</p>
        <p className="splash-verbs" aria-label={t("landing.hero.verbs")}>
          {(["architect", "build", "test", "secure", "deploy"] as const).map((verb) => (
            <span key={verb}>{t(`landing.hero.${verb}`)}</span>
          ))}
        </p>
        ...
        {init.phase === "error" ? (
          <div className="splash-error lp-glass" role="alert">
            ...
            <div>
              <p className="splash-error__title">{t("splash.errorTitle")}</p>
              <p className="splash-error__text">{t("splash.errorText")}</p>
            </div>
      ...
      <section className="splash-modules lp-glass" aria-label={t("splash.modules")}>
        <p className="splash-modules__title">{t("splash.modules")}</p>
        <ul>
          {init.stages.map((stage) => (
            <li key={stage.id} className={`splash-stage-row is-${stage.status}`}>
              <StageIcon stage={stage} />
              <span className="splash-stage-row__label">{t(`splash.stages.${stage.id}`)}</span>
              <span className="splash-stage-row__detail">{stage.detail ? t(stage.detail) : stage.status === "pending" ? t("splash.waiting") : ""}</span>
            </li>
          ))}
        </ul>
      </section>

      <ul className="splash-capabilities" aria-label={t("splash.capabilities.label")}>

...
interface AdapterDefinition {
  ...
  /** Host OS the family requires (platform truth, not a label). */
  hostOs?: readonly OsName[];
  environmentTypes?: readonly EnvironmentType[];
  /** Toolchains every operation of this family needs, whatever it declares. */
  baseToolchains?: readonly ToolchainKind[];
  /** Extra discovered targets derived from toolchain components. */
  targetEvidence?: (ctx: AdapterContext) => readonly string[];
  extra?: (
    ctx: AdapterContext,
...
function defineAdapter(def: AdapterDefinition): EnvironmentExecutionAdapter {
  return {
    ...
    evaluate(ctx, requirement): AdapterCompatibility {
      ...
      reasons.push(
        ...evaluateTargets(ctx.instance, {
          ...requirement,
          targets: (requirement.targets ?? []).filter(
            (t) => !extraTargets.has(t),
          ),
        }),
      );
...
export function createWindowsAdapter(): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "windows-toolchain",
    family: "windows",
    description:
      "Windows toolchains (.NET SDK, MSBuild, Node) through registered operations; Visual Studio projects route here via the .NET SDK — never via IDE automation.",
...
export function createMacosXcodeAdapter(): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "macos-xcode",
    family: "macos",
    description:
      "Native Apple builds with the Xcode toolchain on an eligible macOS host. Signing and distribution are separate protected capabilities.",
    executables: ["xcodebuild", "swift", "node"],
    hostOs: ["macos"],
    // An installed `<platform>_sdk` Xcode component proves that target.
    targetEvidence: (ctx) =>
      Object.keys(
        ctx.instance.toolchains.find((t) => t.kind === "swift_xcode")
          ?.componentVersions ?? {},
      )
...
export function createAndroidAdapter(): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "android-gradle",
    family: "android",
    description:
      "Android builds through registered Gradle profiles (JDK + Android SDK). APK/AAB signing and Play publishing are never implied.",
...
export function createLinuxAdapter(): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "linux-toolchain",
    family: "linux",
    description:
      "Linux toolchains (Node, Python, Go, Rust, C/C++) through registered operations.",
...
export function createDockerAdapter(
  ...
  return defineAdapter({
    ...
    family: "docker",
    description:
      "Bounded container workloads with approved images, symbolic workspace mounts and policy-controlled networking.",
    executables: ["docker"],
    extra: (ctx, requirement) => {
      const reasons: ExecutionReason[] = [];
      const runtime = ctx.instance.capabilities.some(
        (c) => c.capability === "container_runtime_available" && c.available,
      );
      if (!runtime) {
        reasons.push({
          code: "TOOLCHAIN_MISSING",
        ...
        readExecutionDiscovery(ctx.instance).containerDaemon !== "operational"
      ) {
        reasons.push({
          code: "ENVIRONMENT_OFFLINE",
          detail:
            "the container daemon is not operational (runtime known ≠ execution ready)",
        });
      }
      if (!requirement.container) {
        ...
        reasons.push(
          ...evaluateContainerRequest(requirement.container, policy),
        );
      }
      return reasons;
    },
 ...
 */
export function createCloudRunnerAdapter(
  executables: readonly string[] = ["node"],
): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "cloud-runner",
    family: "cloud",
    description:
      "Provider-neutral remote runners with verified identity, leases and heartbeats. No endpoint is ever taken from a request.",
    executables,
    environmentTypes: ["cloud_runner"],
    extra: (ctx) =>
      ctx.host.hostType === "cloud_runner"
        ? []
        : [
            {
              code: "PLATFORM_MISMATCH",
...
export function createUnityAdapter(): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "unity-editor",
    family: "unity",
    description:
      "Unity batch-mode builds/tests with explicit editor version matching and discovered target modules.",
    executables: ["unity"],
    baseToolchains: ["unity"],
    extra: (_ctx, requirement) =>
      requirement.exactVersions?.some((v) => v.kind === "unity")
        ? []
        : [
            {
              code: "ADAPTER_ERROR",
...
export function createUnrealAdapter(): EnvironmentExecutionAdapter {
  return defineAdapter({
    adapterId: "unreal-engine",
    family: "unreal",
    description:
      "Unreal Engine builds/automation tests with explicit engine versions, a C++ toolchain and discovered platform SDKs. Projects are never migrated.",
    executables: ["unreal-uat"],
    baseToolchains: ["unreal", "cpp_compiler"],
    extra: (_ctx, requirement) =>
      requirement.exactVersions?.some((v) => v.kind === "unreal")
        ? []
        : [
            {
              code: "ADAPTER_ERROR",
